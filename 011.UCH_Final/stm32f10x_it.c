 ...
 * Return         : None
 *******************************************************************************/
extern const int maple_theme[][2];
extern const unsigned short tone_freq[];
...
void TIM2_IRQHandler(void)
{
    ...
    int tone = maple_theme[note_index][0];
    int duration = maple_theme[note_index][1];

    if (tone != REST) {
      TIM3_Out_Freq_Generation(tone_freq[tone]);  // 유효한 음만 출력
    }
    note_index++;
    if (note_index >= MAPLE_LEN)
        note_index = 0;

    TIM2->ARR = duration * TIME2_PLS_OF_1ms;
